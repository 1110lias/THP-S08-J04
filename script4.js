...
const ageActuelInventeurs = entrepreneurs.map(entrepreneur => {
    const age = new Date().getFullYear() - entrepreneur.year;
    return `${entrepreneur.first} ${entrepreneur.last} aurait ${age} ans aujourd'hui.`;
});
console.log(ageActuelInventeurs);
...
// Question 4 bis: Trie les entrepreneurs par ordre alphabétique du nom de famille
console.log("Entrepreneurs triés par ordre alphabétique du nom de famille :");
const entrepreneursTries = entrepreneurs.sort((a, b) => {
    // Comparaison des noms de famille en minuscules pour éviter les différences de casse
    const nomA = a.last.toLowerCase();
    const nomB = b.last.toLowerCase();

    // Si le nom de famille de a est inférieur à celui de b dans l'ordre alphabétique
    if (nomA < nomB) {
        return -1; // Retourne un nombre négatif pour placer a avant b
    }
    // Si le nom de famille de a est supérieur à celui de b dans l'ordre alphabétique
    if (nomA > nomB) {
        return 1; // Retourne un nombre positif pour placer a après b
    }
    // Si les noms de famille sont égaux
    return 0; // Aucun changement d'ordre entre a et b
});
console.log(entrepreneursTries);